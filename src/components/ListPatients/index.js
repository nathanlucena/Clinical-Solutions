...
export const ListPatients = ({ active }) => {
  ...
  const handleSubmitSearch = (e, item) => {
    ...
    if(e.key === 'Enter'){
      let result = listPatients.filter(patient => patient.name.toLowerCase().includes(value))
      setList(result)
    } 
  }
          ...
          <BootstrapInput
            ...
            startAdornment={
              <InputAdornment position="start">
                <Image src={lupa} alt="Lupa" className="looking-glass" width="20px" />
              </InputAdornment>
            } />
        </FormControl>
      </div>
              ...
              <div key={item.rg} className="block-cards">
                <Card >
                  <Link href={'patient/' + item.rg}>
                    <div className="container-profile" onClick={(e) => {
                      e.stopPropagation
